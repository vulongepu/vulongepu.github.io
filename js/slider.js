...
jQuery.fn.rbtSlider = function(opt){
        var slider = $(this);
        if (opt.height) $(this).css('height', opt.height);
        $(this).find('.slItem').first().addClass('active');
        if (opt.dots) {
            var count = $('.slItem').size();
            slider.append(
                $('<div/>', {
                    class: 'slDots',
                    html: $('<div/>', {
        ...
        function nav(side, index) {
            if (index) {
                ...
                if (side == 'prev') {
                    if (slider.find('.slItem.active').prev().length) nextItem = $('.slItem.active').prev();  
                    else nextItem = slider.find('.slItem').last();
                } else {
                    if (slider.find('.slItem.active').next().length) nextItem = $('.slItem.active').next();
            ...
            nextItem.addClass(side + 'Item').delay(50).queue(function(){
                $('.slItems > .active').addClass(side).delay(700).queue(function(){
